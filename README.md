...
У разных платформ разная глубина интеграции.
Zoom предоставляет официальный контур для recordings, transcripts, webhooks и RTMS. У Zoom также есть webhook-события и RTMS для live audio/transcript. citeturn936743search1turn936743search14turn936743search17

У Телемоста официально доступны запись встречи, а также в бизнес-сценариях встречаются конспектирование/расшифровка, но как MVP безопаснее закладывать fallback-путь через отправку записи или текста в бота. citeturn742843search1turn742843search3turn742843search10

Telegram Bot API официально поддерживает получение файлов и работу через webhook либо long polling, поэтому бот можно сделать как единый интерфейс поверх разных ingestion-адаптеров. citeturn936743search2turn936743search5
...
1. Завести роли: руководитель / секретарь / наблюдатель
2. Подключить planner API
3. Сделать уточнение неуверенных поручений кнопками
4. Довести Zoom OAuth + download recordings
5. Добавить экспорт протокола в PDF/DOCX
